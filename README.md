# Void Linux Setup Guide

## Basic dependencies

### 1. (If dual-boot) Configuring grub bootloader:
- First install os-prober
```bash
sudo xbps-install -S os-prober
```
- Then edit `/etc/default/grub` and update grub configuration:  
```bash
echo "GRUB_DISABLE_OS_PROBER=false" | sudo tee -a /etc/default/grub
sudo grub-mkconfig -o /boot/grub/grub.cfg
```

### 2. NetworkManager and dbus
...
```
- Disable other network services like `wpa_supplicant` and `dhcpcd` using    
```bash
...
```
- After that create a symlink for dbus and NetworkManager, and enable them.
```bash
...
### 1. Install drivers and additional services
```bash
...
### 2. Install niri and some dependencies
- Niri and apps
```bash
...
### 3. To run niri write the following script:
```bash
#!/bin/bash
