...
Singleton {
    ...
    function findCurrentMode(m) {
        ...
        return m.availableModes.find(mode => {
            ...
            return match && match[1] === target && Math.round(parseFloat(match[2])) === rateRounded;
        }) ?? m.availableModes[0] ?? `${m.width}x${m.height}@${m.refreshRate.toFixed(2)}Hz`;
    }
    ...
    function buildFileContent(edits) {
        ...
        for (const m of edits)
            lines.push(buildMonitorLine(m, "monitor="));
        return lines.join("\n") + "\n";
    }
    ...
    function persistToFile(edits) {
        ...
        writeProc.command = ["python3", "-c", `import os; open(os.path.expanduser('~/.config/hypr/monitors.conf'), 'w').write(${JSON.stringify(content)})`];
    ...
    Process {
        id: sourceCheck
        command: ["sh", "-c", "grep -q 'source.*monitors\\.conf' \"$HOME/.config/hypr/hyprland.conf\" && echo yes || echo no"]
        running: true
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                if (text.trim() !== "")
                    console.warn("MonitorService sourceCheck:", text.trim());
            }
        }
    }
    ...
    Process {
                ...
                try {
                    ...
                    root.monitors = raw.map(m => ({
                                ...
                                mirrorOf: (() => {
                                        ...
                                        if (!isNaN(asId))
                                            return raw.find(x => x.id === asId)?.name ?? "";
                                        return m.mirrorOf;
                                    })(),
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                if (text.trim() !== "")
                    console.warn("MonitorService queryProc:", text.trim());
            }
        }
    }
    ...
    Connections {
        ...
        function onRawEvent(event) {
            ...
            if (root._pendingVerify && event.name === "configreloaded")
                return;
