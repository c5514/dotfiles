...
Item {
    ...
    Popup {
        ...
        contentItem: ListView {
            ...
            model: {
                if (panel.monitor === null)
                    return [];
                const opts = [""];
                for (const m of panel.allMonitors) {
                    if (m.name !== panel.monitor.name)
                        opts.push(m.name);
                }
                return opts;
            }
            ...
            delegate: Rectangle {
                ...
                Text {
                    ...
                    color: modelData === (panel.monitor?.mirrorOf ?? "") ? panel.theme.bgBase : (modelData === "" ? panel.theme.textMuted : panel.theme.textPrimary)
                    font {
                        pixelSize: 11
                        family: panel.font
                    }
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        if (panel.monitor !== null)
                            panel.enableToggled(panel.monitor.disabled);
                    }
                }
            }
