...
Scope {
    ...
    function initEditState() {
        ...
        for (const m of raw) {
            if (m.disabled || !(m.x === 0 && m.y === 0))
                continue;
            const others = enabled.filter(o => o.name !== m.name);
            const overlaps = others.some(o => MonitorUtils.overlapsAABB(m.x, m.y, MonitorUtils.logicalW(m), MonitorUtils.logicalH(m), o.x, o.y, MonitorUtils.logicalW(o), MonitorUtils.logicalH(o)));
            if (overlaps)
                m.x = rightEdge;
        }
    ...
    readonly property bool hasOverlap: {
        ...
        for (let i = 0; i < enabled.length; i++) {
            for (let j = i + 1; j < enabled.length; j++) {
                ...
                if (MonitorUtils.overlapsAABB(a.x, a.y, MonitorUtils.logicalW(a), MonitorUtils.logicalH(a), b.x, b.y, MonitorUtils.logicalW(b), MonitorUtils.logicalH(b)))
                    return true;
            }
        }
        return false;
    ...
    function _hasExternalChange() {
        ...
        if (live.length !== _openSnapshot.length)
            return true;
        for (const snap of _openSnapshot) {
            const l = live.find(m => m.name === snap.name);
            if (!l)
                return true;
            if (l.selectedMode !== snap.selectedMode)
                return true;
            if (l.disabled !== snap.disabled)
                return true;
            if (l.x !== snap.x)
                return true;
            if (l.y !== snap.y)
                return true;
            if (l.scale !== snap.scale)
                return true;
            if (l.transform !== snap.transform)
                return true;
        }
        return false;
    }
    ...
    function onModeSelected(mode) {
        if (selectedIndex < 0 || selectedIndex >= editState.length)
            return;
    ...
    Connections {
        ...
        function onMonitorsLoaded() {
            if (MonitorService._pendingVerify)
                return;
            if (!root.isOpen)
                return;
    ...
    PanelWindow {
        ...
        Timer {
            ...
            interval: 3000
            repeat: true
            running: root.isOpen && !MonitorService.loading && !MonitorService._pendingVerify && !canvas.isDragging
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    MonitorCanvas {
                        ...
                        onMonitorMoved: (idx, nx, ny) => {
                            ...
                            copy[idx] = Object.assign({}, root.editState[idx], {
                                x: nx,
                                y: ny
                            });
                            root.editState = copy;
                ...
                RowLayout {
                    ...
                    Rectangle {
                        width: applyText.width + 24
                        height: 32
                        radius: 8
                        color: (root.isApplying || MonitorService.loading || root.hasOverlap) ? root.theme.bgSurface : root.theme.accentPrimary
                        border.color: root.theme.bgBorder
                        border.width: 1
                        ...
                        Text {
                            ...
                            color: (root.isApplying || MonitorService.loading || root.hasOverlap) ? root.theme.textMuted : root.theme.bgBase
                            font {
                                pixelSize: 12
                                bold: true
                                family: root.font
                        ...
                        MouseArea {
                            anchors.fill: parent
                            cursorShape: (root.isApplying || MonitorService.loading || root.hasOverlap) ? Qt.ArrowCursor : Qt.PointingHandCursor
