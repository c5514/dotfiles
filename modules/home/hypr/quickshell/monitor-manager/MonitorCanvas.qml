...
Item {
    ...
    function _onCanvas(m) {
        return !m.disabled && m.mirrorOf === "";
    }
    ...
    readonly property real viewScale: Math.min((width - 80) / Math.max(_totalW, 1), (height - 80 - _stripH) / Math.max(_totalH, 1))

    readonly property real originX: (width - _totalW * viewScale) / 2 - _minX * viewScale
    readonly property real originY: (_stripH === 0 ? (height - _totalH * viewScale) / 2 : (height - _stripH - _totalH * viewScale) / 2) - _minY * viewScale

    function _mirroredBy(name) {
        const r = [];
        for (const m of monitors) {
            if (!m.disabled && m.mirrorOf === name)
                r.push(m.name);
        }
        return r;
    }
    ...
    Canvas {
        ...
        onAvailableChanged: if (available)
            requestPaint()
        onDotColorChanged: if (available)
            requestPaint()
            ...
            Rectangle {
                ...
                visible: {
                    for (let i = 0; i < canvas.monitors.length; i++) {
                        if (i === enabledTile.index)
                            continue;
                        const o = canvas.monitors[i];
                        if (!canvas._onCanvas(o))
                            continue;
                        if (MonitorUtils.overlapsAABB(modelData.x, modelData.y, MonitorUtils.logicalW(modelData), MonitorUtils.logicalH(modelData), o.x, o.y, MonitorUtils.logicalW(o), MonitorUtils.logicalH(o)))
                            return true;
                    }
                    return false;
                }
    ...
    Repeater {
        ...
        model: {
            ...
            return all.map((e, col) => Object.assign({}, e, {
                    col: col
                }));
        }
    ...
    function snapAndCommit(index, rawCanvasX, rawCanvasY) {
        ...
        for (let i = 0; i < monitors.length; i++) {
            if (i === index || !_onCanvas(monitors[i]))
                continue;
            const m = monitors[i];
            ...
            if (!sx) {
                if (Math.abs(lx - (m.x + mW)) < T) {
                    lx = m.x + mW;
                    sx = true;
                } else if (Math.abs(lx + dW - m.x) < T) {
                    lx = m.x - dW;
                    sx = true;
                } else if (Math.abs(lx - m.x) < T) {
                    lx = m.x;
                    sx = true;
                }
            }
            if (!sy) {
                if (Math.abs(ly - (m.y + mH)) < T) {
                    ly = m.y + mH;
                    sy = true;
                } else if (Math.abs(ly + dH - m.y) < T) {
                    ly = m.y - dH;
                    sy = true;
                } else if (Math.abs(ly - m.y) < T) {
                    ly = m.y;
                    sy = true;
                }
            }
        ...
        for (let iter = 0; iter < monitors.length + 2; iter++) {
            ...
            for (let i = 0; i < monitors.length; i++) {
                ...
                if (!MonitorUtils.overlapsAABB(lx, ly, dW, dH, o.x, o.y, oW, oH))
                    continue;
