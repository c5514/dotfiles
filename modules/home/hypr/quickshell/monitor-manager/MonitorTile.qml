...
Item {
    ...
    Rectangle {
        anchors.fill: parent
        radius: 6
        color: root.selected ? Qt.rgba(root.theme.accentPrimary.r, root.theme.accentPrimary.g, root.theme.accentPrimary.b, 0.15) : root.theme.bgBase
        border.color: root.selected ? root.theme.accentPrimary : (root._isMirror && !root.monitor.disabled ? root.theme.accentCyan : root.theme.bgBorder)
        ...
        Text {
            ...
            text: root.monitor.disabled ? "disabled" : (root._isMirror ? "↔ " + root.monitor.mirrorOf : root.monitor.selectedMode.replace("Hz", ""))
            color: (root._isMirror && !root.monitor.disabled) ? root.theme.accentCyan : root.theme.textSecondary
            font {
        ...
        Rectangle {
            ...
            Text {
                ...
                text: root.mirroredBy.length === 1 ? "↔ " + root.mirroredBy[0] : "↔ " + root.mirroredBy.length
    ...
    MouseArea {
        ...
        onReleased: {
            if (drag.active)
                root.dragEnded(root.index, root.x, root.y);
        }
    }
}
